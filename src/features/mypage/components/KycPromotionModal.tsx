...
const KycPromotionModal: React.FC<KycPromotionModalProps> = ({ isOpen, onClose, onStartKyc }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
            ...
            {/* Modal Content */}
            <div className="relative w-full max-w-md bg-[#0D0D0D] border border-[#2B3139] rounded-[2rem] p-8 shadow-2xl animate-in zoom-in-95 slide-in-from-bottom-4 duration-300">
                {/* Close Button */}
                <button
                    onClick={onClose}
                ...
                <div className="flex flex-col items-center text-center gap-6">
                    {/* Icon */}
                    <div className="w-20 h-20 bg-[#593FFF]/10 rounded-3xl flex items-center justify-center text-[#593FFF] animate-bounce">
                        <ShieldCheck className="w-10 h-10" />
                    </div>

                    {/* Text */}
                    <div className="space-y-2">
                        <h2 className="text-2xl font-black tracking-tight text-white">Welcome to Finora!</h2>
                    ...
                    {/* Features List */}
                    <div className="w-full bg-[#000000] rounded-2xl border border-[#2B3139] p-4 flex flex-col gap-3 text-left">
                        {[
                            ...
                            <div key={idx} className="flex items-center gap-3">
                                <div className="w-1.5 h-1.5 rounded-full bg-[#593FFF]"></div>
                    ...
                    <div className="w-full flex flex-col gap-3 pt-2">
                        <button
                            onClick={onStartKyc}
                            className="w-full bg-[#593FFF] text-white py-4 rounded-2xl font-black text-sm hover:opacity-90 transition shadow-xl shadow-[#593FFF]/20 flex items-center justify-center gap-2 group"
                        >
