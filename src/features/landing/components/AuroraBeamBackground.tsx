...
export const AuroraBeamBackground: React.FC<{ className?: string }> = ({ className }) => {
    const canvasRef = useRef<HTMLCanvasElement>(null);
    const targetMouse = useRef({ x: 0, y: 0 });
    const currentMouse = useRef({ x: 0, y: 0 });
    const animationFrameId = useRef<number>(0);

    // Updated colors to Purple, Opera Pink, Mint, and Cyan (from 컬러-수정1 version)
    ...
    useEffect(() => {
        ...
        const initBeams = () => {
            ...
            for (let i = 0; i < numBeams; i++) {
                ...
                beams.push({
                    ...
                    y: random(0.2, 1.0),
                    z: z,
                    // Extremely tall, thinner width to create "streaks" (from 컬러-수정1 version)
        ...
        const animate = () => {
            ...
            // Pure black base for contrast (consistent across versions)
