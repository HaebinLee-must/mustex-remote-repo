...
const RecentTradesPanel: React.FC<RecentTradesPanelProps> = ({ trades, loading }) => {
  if (loading) {
    return <div className="p-4 text-dark-muted animate-pulse font-bold text-xs uppercase">Loading Trades...</div>;
  }

  return (
    <div className="flex flex-col h-full bg-dark-main text-[11px] font-medium tabular">
      {/* Header */}
      <div className="grid grid-cols-3 px-4 py-2 text-dark-muted border-b border-dark-border font-bold uppercase tracking-wider">
        <span>Price</span>
